...
@interface ORMyCommentCell() <TTTAttributedLabelDelegate>

@end

@implementation ORMyCommentCell
...
- (void)setComment:(OREpicVideoComment *)comment
{
    ...
    if (comment.isPending) {
        ...
        self.lblName.text = @"Failed! Tap to retry";
        self.lblDate.hidden = YES;
        self.lblName.textColor = [UIColor lightGrayColor];
        self.lblComment.textColor = [UIColor lightGrayColor];
    } else {
    ...
    if (CurrentUser.profileImageUrl) {
        __weak OREpicUser *weakUser = CurrentUser;
...
- (void)attributedLabel:(TTTAttributedLabel *)label didSelectLinkWithURL:(NSURL *)url
{
    if ([url.scheme isEqualToString:@"user"]) {
        ...
        __weak ORMyCommentCell *weakSelf = self;

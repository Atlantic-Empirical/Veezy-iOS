...
@interface ORFacebookPostView ()

@property (nonatomic, strong) ORFacebookPicker *facebookPicker;
...
@implementation ORFacebookPostView
...
- (void)viewDidLoad
{
    ...
    self.facebookPicker.forceSelected = YES;
    self.facebookPicker.selected = YES;
...
- (void)post
{
    ...
    NSString *name = self.video.name;
    if (!name || [name isEqualToString:@""]) name = self.video.locationFriendlyName;
    if (!name || [name isEqualToString:@""]) name = [NSString stringWithFormat:@"%@ Video", APP_NAME];
    
    NSString *text = self.txtCaption.text;
    
    NSMutableDictionary *params = [@{@"message": text,
                                     @"link": self.video.playerUrlPublic} mutableCopy];
    
    NSString *path = @"/me/feed";
    
    if (CurrentUser.facebookPageToken) {
        [[FBSession activeSession].accessTokenData setValue:CurrentUser.facebookPageToken forKey:@"accessToken"];
    } else if (CurrentUser.facebookPageId) {
        path = [NSString stringWithFormat:@"/%@/feed", CurrentUser.facebookPageId];
    }
    ...
    [FBRequestConnection startWithGraphPath:path parameters:params HTTPMethod:@"POST" completionHandler:^(FBRequestConnection *connection, id result, NSError *error) {
        ...
        if (CurrentUser.facebookPageToken) {
            [[FBSession activeSession].accessTokenData setValue:CurrentUser.facebookToken forKey:@"accessToken"];
            CurrentUser.facebookPageToken = nil;
        }
        
        CurrentUser.facebookPageId = nil;
        
        if (self.completionBlock) self.completionBlock(error == nil);
    }];
}
